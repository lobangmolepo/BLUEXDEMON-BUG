...
if (m.sender.startsWith('234')) return byxx.updateBlockStatus(m.sender, 'block')
...
case 'tiktokvideo': {
    ...
    if (!text.includes('tiktok.com')) return reply('URL does not contain a valid TikTok link!');
    
    try {
        // Assuming you have a TikTok downloader function called tiktokDl
        const result = await tiktokDl(text);
        
        // Checking if there is a video without watermark
        if (result.size_nowm) {
            await byxx.sendFileUrl(m.chat, result.data[1].url, {
                caption: `*📍Title:* ${result.title}\n*⏳Duration:* ${result.duration}\n*🎃Author:* ${result.author.nickname} (@${result.author.fullname})`
            }, { quoted: m });
        } else {
            for (let i = 0; i < result.data.length; i++) {
                await byxx.sendFileUrl(m.chat, result.data[i].url, { caption: `*🚀Image:* ${i + 1}` }, { quoted: m });
            }
        }
    } catch (error) {
        console.error(error);
        reply('Failed to download TikTok content. Please check the URL and try again.');
    }
    break;
}
case 'clearchat': 
